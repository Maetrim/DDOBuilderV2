...
size_t AbilitySpend::LastPointsSpentCost(
        AbilityType ability) const
{
    size_t lastCost = 0;
    size_t currentSpend = GetAbilitySpend(ability);
    if (currentSpend > 0)
    {
        lastCost = c_statIncreaseCosts[currentSpend-1];
    }
    return lastCost;
}
...
void AbilitySpend::SetAvailableSpend(size_t buildPoints)
{
    Set_AvailableSpend(buildPoints);
}

void AbilitySpend::SetChampion()
{
    m_hasChampion = true;
}

void AbilitySpend::ClearChampion()
{
    m_hasChampion = false;
}
