...
size_t Dice::MaxSize() const
{
    size_t size = 0;
    if (HasNumber()) size = max(size, m_Number.size());
    if (HasSides()) size = max(size, m_Sides.size());
    if (HasBonus()) size = max(size, m_Bonus.size());
    return size;
}
...
std::string Dice::Description(size_t numStacks) const
{
    ...
    if (m_hasDamage)
    {
        ss << " " << EnumEntryText(m_Damage, damageTypeMap);
    }
    if (HasScalesWithMeleePower()
            || HasScalesWithRangedPower()
            || HasScalesWithSpellPower())
