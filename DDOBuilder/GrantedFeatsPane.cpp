        ...
        if (pBuild != NULL)
        {
            for (auto&& gfit : m_grantedFeats)
            {
                ...
                if (feat.Name() == featName
                        && feat.RequirementsToTrain().Met(*pBuild, pBuild->Level(), true, Weapon_Unknown, Weapon_Unknown))
                {
                    isGranted = true;
                }
            }
    ...
    if (m_pCharacter != NULL)
    {
        ...
        if (pBuild != NULL)
        {
            for (auto&& gfit : m_grantedFeats)
            {
                ...
                if (feat.RequirementsToTrain().Met(*pBuild, pBuild->Level(), true, Weapon_Unknown, Weapon_Unknown))
                {
                    // its active
...
void CGrantedFeatsPane::AddGrantedFeatEffect(const Effect& effect)
{
    ...
    if (!bFound)
    {
        ...
        if (effect.HasRequirementsToBeActive()
                && !effect.RequirementsToBeActive().Met(*pBuild, pBuild->Level(), true, Weapon_Unknown, Weapon_Unknown))
        {
