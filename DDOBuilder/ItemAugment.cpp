...
bool ItemAugment::VerifyObject(std::stringstream * ss) const
{
    ...
    //std::vector<Augment> augments = CompatibleAugments(Type(), size_t maxLevel);
    //if (augments.size() == 0)
    //{
...
const Augment& ItemAugment::GetSelectedAugment() const
{
    static Augment badAugment;
    if (HasSelectedAugment())
    {
        if (ItemSpecificAugments().size() > 0)
        {
            for (auto&& isait : m_ItemSpecificAugments)
            {
                if (isait.Name() == SelectedAugment())
                {
                    return isait;
                }
            }
