...
LRESULT CTwistOfFateDialog::OnNewDocument(WPARAM, LPARAM lParam)
{
...
void CTwistOfFateDialog::SetupControls()
{
    ...
    {
        if (m_pCharacter->ActiveBuild()->IsTwistActive(m_twistIndex))
        {
            ...
            if (m_twistIndex < MAX_TWISTS - 1)
            {
                ...
                canDecrement = m_pCharacter->ActiveBuild()->TwistTier(m_twistIndex)
                        > m_pCharacter->ActiveBuild()->TwistTier(m_twistIndex+1);
            }
            m_buttonMinus.EnableWindow(
                    m_pCharacter->ActiveBuild()->TwistTier(m_twistIndex) > 0
                    && canDecrement);
            m_comboTwistSelect.EnableWindow(m_pCharacter->ActiveBuild()->TwistTier(m_twistIndex) > 0);
            m_buttonPlus.EnableWindow(m_pCharacter->ActiveBuild()->CanUpgradeTwist(m_twistIndex));
            // show current tier and next upgrade cost
            CString text;
            if (m_pCharacter->ActiveBuild()->TwistTier(m_twistIndex) < MAX_TWIST_LEVEL)
            {
                size_t nextCost = 1 + m_twistIndex + m_pCharacter->ActiveBuild()->TwistTier(m_twistIndex);
                text.Format("Tier %d (Cost %d)",
                        m_pCharacter->ActiveBuild()->TwistTier(m_twistIndex),
                        nextCost);
            }
            else
            {
                // just show the current tier
                text.Format("Tier %d (MAX)", m_pCharacter->ActiveBuild()->TwistTier(m_twistIndex));
            }
...
void CTwistOfFateDialog::PopulateTwistCombobox()
{
    ...
    const TrainedEnhancement * trainedTwist = NULL;
    m_availableTwists.clear();
    if (m_pCharacter->ActiveBuild() != NULL)
    {
        ...
        m_availableTwists = m_pCharacter->ActiveBuild()->AvailableTwists(m_twistIndex);
        trainedTwist = m_pCharacter->ActiveBuild()->TrainedTwist(m_twistIndex);
    }
    ...
    while (it != m_availableTwists.end())
    {
        ...
        HRESULT result = LoadImageFile(
                ...
                item != NULL ? item->ActiveIcon(*m_pCharacter->ActiveBuild()) : "NoTwist",
                &image,
                CSize(32, 32));
        if (result == S_OK)
        {
    ...
    if (trainedTwist != NULL && !foundTwist)
    {
        ...
        m_pCharacter->ActiveBuild()->SetTwist(m_twistIndex, NULL);
        // let the user know
...
{
    m_pCharacter->ActiveBuild()->DowngradeTwist(m_twistIndex);
}

void CTwistOfFateDialog::OnButtonPlus()
{
    m_pCharacter->ActiveBuild()->UpgradeTwist(m_twistIndex);
}
...
void CTwistOfFateDialog::OnComboTwistSelect()
{
    ...
    if (sel != CB_ERR)
    {
        ...
        std::advance(it, sel);
        m_pCharacter->ActiveBuild()->SetTwist(m_twistIndex, &(*it));
    }
}

LRESULT CTwistOfFateDialog::OnHoverComboBox(WPARAM wParam, LPARAM)
{
